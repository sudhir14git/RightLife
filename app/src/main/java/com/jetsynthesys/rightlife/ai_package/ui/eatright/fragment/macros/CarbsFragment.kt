...
class CarbsFragment : BaseFragment<FragmentCarbsBinding>() {
    ...
    override val bindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentCarbsBinding
        get() = FragmentCarbsBinding::inflate
...
//    }
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedCarbsResponse, currentDate: String
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedCarbsResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedCarbsResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumedCarbsTotals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.carbsConsumed.toFloat() ?: 0f)
                    }
                }
            }
        }
