...
class VitaminAFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
                            activeCaloriesResponse: ConsumedVitaminAResponse) {
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.total_vitamin_a.toFloat(),
                activeCaloriesResponse.current_avg_vitamin_a.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.total_vitamin_a.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.current_avg_vitamin_a.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
                ...
                val response = ApiClient.apiServiceFastApiV2.getConsumedVitaminA(
                    userId = userId,
                    period = period,
                    date = selectedDate
                )
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedVitaminAResponse, currentDate: String
        ...
        if (activeCaloriesResponse.consumed_vitamin_a_totals.isNotEmpty()){
            activeCaloriesResponse.consumed_vitamin_a_totals.forEach { calorie ->
                val startDate = dateFormat.parse(calorie.date)?.let { Date(it.time) }
                if (startDate != null) {
                    val dayKey = dateFormat.format(startDate)
                    calorieMap[dayKey] = calorieMap[dayKey]!! + (calorie.vitamin_a_consumed.toFloat() ?: 0f)
                }
            }
        }
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedVitaminAResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
            ...
            activeCaloriesResponse.consumed_vitamin_a_totals.forEach { calorie ->
                ...
                if (parsedDate != null) {
                    ...
                    if (calorieMap.containsKey(dayKey)) {
                        ...
                        val newValue = calorie.vitamin_a_consumed.toFloat() ?: 0f
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedVitaminAResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumed_vitamin_a_totals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.vitamin_a_consumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedVitaminAResponse, type: String) {
        activity?.runOnUiThread {
            ...
            totalCalorie.text = activeCaloriesResponse.total_vitamin_a.toInt().toString()
