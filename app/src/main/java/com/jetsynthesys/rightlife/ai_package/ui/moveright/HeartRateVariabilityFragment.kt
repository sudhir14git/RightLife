...
class HeartRateVariabilityFragment : BaseFragment<FragmentHeartRateVariabilityBinding>() {
    ...
    private fun processWeeklyData(data: HeartRateVariabilityResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val calendar = Calendar.getInstance()
        ...
        data.activeHrvTotals?.forEach { hrv ->
            val dayKey = hrv.date
            if (hrvMap.containsKey(dayKey)) {
                hrvMap[dayKey] = hrv.hrv?.toFloat() ?: 0f
            }
        }
    ...
    private fun processMonthlyData(data: HeartRateVariabilityResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
        ...
        data.activeHrvTotals?.forEach { hrv ->
            ...
            if (weekIndex in 0..4) {
                weeklyHrvMap[weekIndex] = hrv.hrv?.toFloat() ?: 0f
            }
        }
    ...
    private fun processSixMonthsData(data: HeartRateVariabilityResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        ...
        data.activeHrvTotals?.forEach { hrv ->
            ...
            if (monthIndex in 0..5) {
                hrvMap[monthIndex] = hrv.hrv?.toFloat() ?: 0f
            }
        }
    ...
    private fun setLastAverageValue(data: HeartRateVariabilityResponse, period: String) {
        activity?.runOnUiThread {
            val averageHrv = data.currentAvgHrv?.toInt() ?: 0
