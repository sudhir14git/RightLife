...
class VitaminCFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
                            activeCaloriesResponse: ConsumedVitaminCResponse) {
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.total_vitamin_c.toFloat(),
                activeCaloriesResponse.current_avg_vitamin_c.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.total_vitamin_c.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.current_avg_vitamin_c.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
                ...
                val response = ApiClient.apiServiceFastApiV2.getConsumedVitaminC(
                    userId = userId,
                    period = period,
                    date = selectedDate
                )
                    ...
                    if (activeCaloriesResponse?.statusCode == 200) {
                        activeCaloriesResponse?.let { data ->
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                val totalCalories = data.consumed_vitamin_c_totals?.sumOf { it.vitamin_c_consumed ?: 0.0 } ?: 0.0
                                if (data.consumed_vitamin_c_totals?.size ?: 0 > 31) {
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedVitaminCResponse, currentDate: String
        ...
        if (activeCaloriesResponse.consumed_vitamin_c_totals.isNotEmpty()){
            activeCaloriesResponse.consumed_vitamin_c_totals.forEach { calorie ->
                val startDate = dateFormat.parse(calorie.date)?.let { Date(it.time) }
                if (startDate != null) {
                    val dayKey = dateFormat.format(startDate)
                    calorieMap[dayKey] = calorieMap[dayKey]!! + (calorie.vitamin_c_consumed.toFloat() ?: 0f)
                }
            }
        }
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedVitaminCResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
            ...
            activeCaloriesResponse.consumed_vitamin_c_totals.forEach { calorie ->
                ...
                if (parsedDate != null) {
                    ...
                    if (calorieMap.containsKey(dayKey)) {
                        ...
                        val newValue = calorie.vitamin_c_consumed.toFloat() ?: 0f
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedVitaminCResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumed_vitamin_c_totals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.vitamin_c_consumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedVitaminCResponse, type: String) {
        activity?.runOnUiThread {
            averageBurnCalorie.text = activeCaloriesResponse.current_avg_vitamin_c.toInt().toString()
            totalCalorie.text = activeCaloriesResponse.total_vitamin_c.toInt().toString()
