...
class VitaminDFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
                            activeCaloriesResponse: ConsumedVitaminDResponse) {
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.total_vitamin_d.toFloat(),
                activeCaloriesResponse.current_avg_vitamin_d.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.total_vitamin_d.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.current_avg_vitamin_d.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
                ...
                val response = ApiClient.apiServiceFastApiV2.getConsumedVitaminD(
                    userId = userId,
                    period = period,
                    date = selectedDate
                )
                    ...
                    if (activeCaloriesResponse?.statusCode == 200) {
                        activeCaloriesResponse?.let { data ->
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                val totalCalories = data.consumed_vitamin_d_totals?.sumOf { it.vitamin_d_consumed ?: 0.0 } ?: 0.0
                                if (data.consumed_vitamin_d_totals?.size ?: 0 > 31) {
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedVitaminDResponse, currentDate: String
        ...
        if (activeCaloriesResponse.consumed_vitamin_d_totals.isNotEmpty()){
            activeCaloriesResponse.consumed_vitamin_d_totals.forEach { calorie ->
                val startDate = dateFormat.parse(calorie.date)?.let { Date(it.time) }
                if (startDate != null) {
                    val dayKey = dateFormat.format(startDate)
                    calorieMap[dayKey] = calorieMap[dayKey]!! + (calorie.vitamin_d_consumed.toFloat() ?: 0f)
                }
            }
        }
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedVitaminDResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
            ...
            activeCaloriesResponse.consumed_vitamin_d_totals.forEach { calorie ->
                ...
                if (parsedDate != null) {
                    ...
                    if (calorieMap.containsKey(dayKey)) {
                        ...
                        val newValue = calorie.vitamin_d_consumed.toFloat() ?: 0f
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedVitaminDResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumed_vitamin_d_totals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.vitamin_d_consumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedVitaminDResponse, type: String) {
        activity?.runOnUiThread {
            ...
            percentageIc.text = "μg"
            totalPercentageIc.text = "μg"
