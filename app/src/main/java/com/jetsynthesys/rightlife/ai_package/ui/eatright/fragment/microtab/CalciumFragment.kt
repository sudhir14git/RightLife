...
class CalciumFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
                            activeCaloriesResponse: ConsumedCalciumResponse) {
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.total_calcium.toFloat(),
                activeCaloriesResponse.current_avg_calcium.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.total_calcium.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.current_avg_calcium.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
                    ...
                    if (activeCaloriesResponse?.statusCode == 200) {
                        activeCaloriesResponse?.let { data ->
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                val totalCalories = data.consumed_calcium_totals?.sumOf { it.calcium_consumed ?: 0.0 } ?: 0.0
                                if (data.consumed_calcium_totals?.size ?: 0 > 31) {
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedCalciumResponse, currentDate: String
        ...
        if (activeCaloriesResponse.consumed_calcium_totals.isNotEmpty()){
            activeCaloriesResponse.consumed_calcium_totals.forEach { calorie ->
                val startDate = dateFormat.parse(calorie.date)?.let { Date(it.time) }
                if (startDate != null) {
                    val dayKey = dateFormat.format(startDate)
                    calorieMap[dayKey] = calorieMap[dayKey]!! + (calorie.calcium_consumed.toFloat() ?: 0f)
                }
            }
        }
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedCalciumResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
            ...
            activeCaloriesResponse.consumed_calcium_totals.forEach { calorie ->
                ...
                if (parsedDate != null) {
                    ...
                    if (calorieMap.containsKey(dayKey)) {
                        ...
                        val newValue = calorie.calcium_consumed.toFloat() ?: 0f
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedCalciumResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumed_calcium_totals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.calcium_consumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedCalciumResponse, type: String) {
        activity?.runOnUiThread {
            averageBurnCalorie.text = activeCaloriesResponse.current_avg_calcium.toInt().toString()
            totalCalorie.text = activeCaloriesResponse.total_calcium.toInt().toString()
