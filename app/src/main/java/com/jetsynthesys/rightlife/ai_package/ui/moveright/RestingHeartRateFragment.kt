...
class RestingHeartRateFragment : BaseFragment<FragmentRestingHeartRateBinding>() {
    ...
    private fun processWeeklyData(data: RestingHeartRateResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val calendar = Calendar.getInstance()
        ...
        data.restingHeartRateTotals?.forEach { hrv ->
            val dayKey = hrv.date
            if (hrvMap.containsKey(dayKey)) {
                hrvMap[dayKey] = hrv.bpm?.toFloat() ?: 0f
            }
        }
    ...
    private fun processMonthlyData(data: RestingHeartRateResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
        ...
        data.restingHeartRateTotals?.forEach { hrv ->
            ...
            if (weekIndex in 0..4) {
                weeklyHrvMap[weekIndex] = hrv.bpm?.toFloat() ?: 0f
            }
        }
    ...
    private fun processSixMonthsData(data: RestingHeartRateResponse, currentDate: String): Triple<List<Entry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        ...
        data.restingHeartRateTotals?.forEach { hrv ->
            ...
            if (monthIndex in 0..5) {
                hrvMap[monthIndex] = hrv.bpm?.toFloat() ?: 0f
            }
        }
    ...
    private fun setLastAverageValue(data: RestingHeartRateResponse, period: String) {
        activity?.runOnUiThread {
            val averageHrv = data.currentAvgBpm?.toInt() ?: 0
