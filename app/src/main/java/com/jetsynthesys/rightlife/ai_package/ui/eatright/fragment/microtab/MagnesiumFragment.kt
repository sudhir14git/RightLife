...
class MagnesiumFragment : BaseFragment<FragmentMagnesiumBinding>() {
    ...
    override val bindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentMagnesiumBinding
        get() = FragmentMagnesiumBinding::inflate
...
//                        selectedDate = firstDateOfMonth
                    ...
                    if (activeCaloriesResponse?.statusCode == 200){
                        activeCaloriesResponse.let { data ->
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                magnesium_description_text.text = formatMarkdownBold(data.description)
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedMagnesiumResponse, currentDate: String
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedMagnesiumResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedMagnesiumResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumedMagnesiumTotals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.magnesiumConsumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun lineChartForSixMonths() {
        ...
        lineChart.post {
            ...
            val hMargin = 40f
            val vMargin = 20f
            if (viewModel.currentRange == RangeTypeChartsMagnesium.SIX_MONTHS) {
...
class ActiveBurnViewModelMagnesium : ViewModel() {
    val allDailySteps = mutableListOf<DailyStepMagnesium>()
    val goalSteps = 10000
    var currentRange = RangeTypeChartsMagnesium.SIX_MONTHS
    var startDate = Date()
    var endDate = Date()

    init {
        generateMockData()
        setInitialRange(RangeTypeChartsMagnesium.SIX_MONTHS)
    }
    ...
    private fun generateMockData() {
        ...
        while (current <= end) {
            ...
            allDailySteps.add(DailyStepMagnesium(current, randomSteps))
    ...
    val filteredData: List<DailyStepMagnesium>
        get() = allDailySteps.filter { it.date >= startDate && it.date <= endDate }

    val monthlyGroups: List<MonthGroupsMagnesium>
        get() = if (currentRange != RangeTypeChartsMagnesium.SIX_MONTHS || filteredData.isEmpty()) emptyList() else {
            val calendar = Calendar.getInstance()
            ...
            var results = mutableListOf<MonthGroupsMagnesium>()
            ...
            for ((_, days) in sorted) {
                ...
                results.add(MonthGroupsMagnesium(sortedDays, avg, diffString, isPositive, clampedStart, clampedEnd))
            }
            results
        }

    fun setInitialRange(range: RangeTypeChartsMagnesium) {
        currentRange = range
        val calendar = Calendar.getInstance()
        val now = Date()
        when (range) {
            RangeTypeChartsMagnesium.WEEK -> {
            ...
            RangeTypeChartsMagnesium.MONTH -> {
                calendar.time = now
            ...
            RangeTypeChartsMagnesium.SIX_MONTHS -> {
                calendar.time = now
    ...
    fun goLeft() {
        ...
        calendar.time = startDate
        when (currentRange) {
            RangeTypeChartsMagnesium.WEEK -> calendar.add(Calendar.DAY_OF_YEAR, -7)
            RangeTypeChartsMagnesium.MONTH -> calendar.add(Calendar.MONTH, -1)
            RangeTypeChartsMagnesium.SIX_MONTHS -> calendar.add(Calendar.MONTH, -6)
        }
        startDate = calendar.time
...
data class DailyStepMagnesium(val date: Date, val steps: Int)
enum class RangeTypeChartsMagnesium { WEEK, MONTH, SIX_MONTHS }
data class MonthGroupsMagnesium(
    val days: List<DailyStepMagnesium>,
    val avgSteps: Int,
...
private fun List<DailyStepMagnesium>.averageSteps(): Number {
    return if (isEmpty()) 0.0 else sumOf { it.steps } / size
}

class CurvedBarChartRendererMagnesium(
    chart: BarChart,
