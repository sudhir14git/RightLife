...
class SodiumFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
                            activeCaloriesResponse: ConsumedSodiumResponse) {
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.total_sodium.toFloat(),
                activeCaloriesResponse.current_avg_sodium.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.total_sodium.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.current_avg_sodium.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
                    ...
                    if (activeCaloriesResponse?.statusCode == 200) {
                        activeCaloriesResponse?.let { data ->
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                val totalCalories = data.consumed_sodium_totals?.sumOf { it.sodium_consumed ?: 0.0 } ?: 0.0
                                if (data.consumed_sodium_totals?.size ?: 0 > 31) {
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedSodiumResponse, currentDate: String
        ...
        if (activeCaloriesResponse.consumed_sodium_totals.isNotEmpty()){
            activeCaloriesResponse.consumed_sodium_totals.forEach { calorie ->
                val startDate = dateFormat.parse(calorie.date)?.let { Date(it.time) }
                if (startDate != null) {
                    val dayKey = dateFormat.format(startDate)
                    calorieMap[dayKey] = calorieMap[dayKey]!! + (calorie.sodium_consumed.toFloat() ?: 0f)
                }
            }
        }
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedSodiumResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
            ...
            activeCaloriesResponse.consumed_sodium_totals.forEach { calorie ->
                ...
                if (parsedDate != null) {
                    ...
                    if (calorieMap.containsKey(dayKey)) {
                        ...
                        val newValue = calorie.sodium_consumed.toFloat() ?: 0f
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedSodiumResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumed_sodium_totals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.sodium_consumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedSodiumResponse, type: String) {
        activity?.runOnUiThread {
            averageBurnCalorie.text = activeCaloriesResponse.current_avg_sodium.toInt().toString()
            totalCalorie.text = activeCaloriesResponse.total_sodium.toInt().toString()
