...
class SugarFragment : BaseFragment<FragmentSugarBinding>() {
    ...
    private fun updateChart(entries: List<BarEntry>, labels: List<String>, labelsDate: List<String>,
        ...
        if (entries.size < 30){
            val minValue = minOf(
                ...
                activeCaloriesResponse.totalSugar.toFloat(),
                activeCaloriesResponse.currentAvgSugar.toFloat()
            )
            val maxValue = maxOf(
            ...
            val axisMax = maxOf(maxValue, activeCaloriesResponse.totalSugar.toFloat())
            ...
            val avgStepsLine = LimitLine(activeCaloriesResponse.currentAvgSugar.toFloat(), "A")
...
//                            selectedDate = firstDateOfMonth
    ...
    private fun processWeeklyData(
        activeCaloriesResponse: ConsumedSugarResponse, currentDate: String
    ...
    private fun processMonthlyData(activeCaloriesResponse: ConsumedSugarResponse, currentDate: String
    ): Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd",  Locale.getDefault())
        val calendar = Calendar.getInstance()
        val dateString = currentDate
    ...
    private fun processSixMonthsData(activeCaloriesResponse: ConsumedSugarResponse, currentDate: String):
            Triple<List<BarEntry>, List<String>, List<String>> {
        val dateFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
        val monthFormat = SimpleDateFormat("MMM", Locale.getDefault())
        val calendar = Calendar.getInstance()
            ...
            activeCaloriesResponse.consumedSugarTotals.forEach { calorie ->
                ...
                if (startDate != null) {
                    ...
                    if (monthIndex in 0..5) {
                        calorieMap[monthIndex] = calorieMap[monthIndex]!! + (calorie.sugarConsumed.toFloat() ?: 0f)
                    }
                }
            }
        }
...
//                val lastDateOfMonth = getFirstDateOfMonth(selectedMonthDate, lastDayOfMonth)
    ...
    private fun setLastAverageValue(activeCaloriesResponse: ConsumedSugarResponse, type: String) {
        activity?.runOnUiThread {
            averageBurnCalorie.text = activeCaloriesResponse.currentAvgSugar.toInt().toString()
            totalCalorie.text = activeCaloriesResponse.totalSugar.toInt().toString()
    ...
    private fun lineChartForSixMonths() {
        ...
        lineChart.post {
            ...
            val hMargin = 40f
            val vMargin = 20f
            if (viewModel.currentRange == RangeTypeChartsSugar.SIX_MONTHS) {
...
class ActiveBurnViewModelSugar : ViewModel() {
    val allDailySteps = mutableListOf<DailyStepSugar>()
    val goalSteps = 10000
    var currentRange = RangeTypeChartsSugar.SIX_MONTHS
    var startDate = Date()
    var endDate = Date()

    init {
        generateMockData()
        setInitialRange(RangeTypeChartsSugar.SIX_MONTHS)
    }
    ...
    private fun generateMockData() {
        ...
        while (current <= end) {
            ...
            allDailySteps.add(DailyStepSugar(current, randomSteps))
    ...
    val filteredData: List<DailyStepSugar>
        get() = allDailySteps.filter { it.date >= startDate && it.date <= endDate }

    val monthlyGroups: List<MonthGroupsSugar>
        get() = if (currentRange != RangeTypeChartsSugar.SIX_MONTHS || filteredData.isEmpty()) emptyList() else {
            val calendar = Calendar.getInstance()
            ...
            var results = mutableListOf<MonthGroupsSugar>()
            ...
            for ((_, days) in sorted) {
                ...
                results.add(MonthGroupsSugar(sortedDays, avg, diffString, isPositive, clampedStart, clampedEnd))
            }
            results
        }

    fun setInitialRange(range: RangeTypeChartsSugar) {
        currentRange = range
        val calendar = Calendar.getInstance()
        val now = Date()
        when (range) {
            RangeTypeChartsSugar.WEEK -> {
            ...
            RangeTypeChartsSugar.SIX_MONTHS -> {
                calendar.time = now
    ...
    fun goLeft() {
        ...
        calendar.time = startDate
        when (currentRange) {
            RangeTypeChartsSugar.WEEK -> calendar.add(Calendar.DAY_OF_YEAR, -7)
            RangeTypeChartsSugar.MONTH -> calendar.add(Calendar.MONTH, -1)
            RangeTypeChartsSugar.SIX_MONTHS -> calendar.add(Calendar.MONTH, -6)
        }
        startDate = calendar.time
...
data class DailyStepSugar(val date: Date, val steps: Int)
enum class RangeTypeChartsSugar { WEEK, MONTH, SIX_MONTHS }
data class MonthGroupsSugar(
    val days: List<DailyStepSugar>,
    val avgSteps: Int,
...
private fun List<DailyStepSugar>.averageSteps(): Number {
    return if (isEmpty()) 0.0 else sumOf { it.steps } / size
}

class CurvedBarChartRendererSugar(
    chart: BarChart,
